...
pub fn router(state: FcAppState) -> Router {
    Router::new()
...
async fn handler_get_config(State(state): State<FcAppState>) -> impl IntoResponse {
...
pub async fn start_server(bind_addr: SocketAddr, state: FcAppState) -> Result<(), String> {
    ...
    log::info!("FC dashboard listening on https://{bind_addr}");
